<?php
// Impede que o browser restaure posição de scroll ao pressionar F5
...
<head>
  ...
  <title><?php
    $titles = ['comunicados'=>'Comunicados','noticias'=>'Notícias Externas','sistemas'=>'Sistemas','post'=>''];
  ...
  <script>if('scrollRestoration'in history)history.scrollRestoration='manual';window.scrollTo(0,0);window.addEventListener('pageshow',function(){window.scrollTo(0,0);});</script>
...
<nav class="navbar">
  ...
  <ul class="navbar-nav">
    <?php $navMenuCurrentPage = $page; $navMenuPublic = true; include __DIR__ . '/includes/nav_menu.php'; ?>
  </ul>
...
<?php /* ══════════════════════════════════════════════
   HOME — igual ao index.php logado
══════════════════════════════════════════════ */ ?>
<?php if ($page === 'home'): ?>

  <!-- Hero — nome da unidade em destaque -->
  <?php
  // hero_title: nome da unidade (configurável em Admin → Configurações)
  // Se não preenchido, usa site_tagline (ex: Unidade de Saúde)
  ...
  <div class="hero" style="padding:20px 0">
    <div class="container hero-content">
      <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:16px">
        ...
        <!-- Links rápidos públicos no hero -->
  ...
  <div class="container main-content">
    ...
    <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius);padding:12px 18px;margin-bottom:12px;display:flex;align-items:center;gap:10px;overflow:hidden">
      ...
      <!-- Chips numa linha, cortados pelo overflow -->
      <div style="flex:1;display:flex;gap:6px;overflow:hidden;min-width:0">
        ...
        <a href="<?= htmlspecialchars($sys['url']) ?>" target="<?= $sys['target'] ?>"
          ...
          <div style="width:18px;height:18px;border-radius:4px;background:<?= htmlspecialchars($sys['color']) ?>20;display:flex;align-items:center;justify-content:center;flex-shrink:0">
      ...
      <!-- Botão ver todos sempre visível -->
      <a href="public.php?page=sistemas"
    ...
    <div class="post-featured">
        <div class="pf-image" style="aspect-ratio:<?= htmlspecialchars($featured['cover_aspect_ratio'] ?? '1.33') ?>/1">
          ...
          <div class="video-thumb-preview" style="height:100%;border-radius:0;aspect-ratio:unset"
            ...
            <?php if ($featYtId): ?>
            <img src="https://img.youtube.com/vi/<?= $featYtId ?>/maxresdefault.jpg" onerror="this.src='https://img.youtube.com/vi/<?= $featYtId ?>/hqdefault.jpg'" alt="" style="width:100%;height:100%;object-fit:cover">
            <?php else: ?><div class="video-thumb-placeholder"><span class="material-icons">play_circle</span></div><?php endif; ?>
        ...
        <a href="public.php?page=post&slug=<?= urlencode($featured['slug']) ?>" class="pf-body" style="text-decoration:none;display:flex;flex-direction:column;justify-content:center">
          ...
          <div style="margin-top:20px;margin-bottom:16px">
            <span style="display:inline-flex;align-items:center;gap:6px;padding:10px 22px;background:var(--primary);color:#fff;border-radius:8px;font-size:13px;font-weight:700;transition:.2s">
    ...
    <!-- ── COMUNICADOS E NOTÍCIAS EXTERNAS LADO A LADO ── -->
...
<?php /* ══════════════════════════════════════════════
   LISTAGEM — /comunicados ou /noticias
══════════════════════════════════════════════ */ ?>
...
<script>
// Garantia: inicializar sliders mesmo que main.js já tenha executado
(function() {
  function tryInit() {
    var sliders = document.querySelectorAll('.post-slider:not([data-init])');
    sliders.forEach(function(el) {
      el.setAttribute('data-init','1');
      if (typeof initPostSlider === 'function') initPostSlider(el);
    });
  }
